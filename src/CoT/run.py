...
def evaluate(
    ...
    if workers <= 1:
        # Sequential with optional streaming
        correct = 0
        total = 0
        errors = 0
        for task_id in ids:
    ...
    db = EvalClient()
    run_id = db.record_baseline_run("CoT", split, all_results, token_usage)
    db.close()
    if run_id is not None:
        log(f"Recorded CoT run {run_id} in DB")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="CoT baseline for ARC")
