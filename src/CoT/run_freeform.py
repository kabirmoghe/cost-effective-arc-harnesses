"""CoT free-form runner — mirror of run.py using the free-form prompt + parser.

Differences vs CoT/run.py:
- Imports prompt_freeform / extract_response_freeform.
- Disables response_format (free-form prose can't be JSON-mode constrained).
- Stamps `extra={"variant": "freeform", "provider": <p>}` on the DB row so it
  is filterable while still living under system='CoT'.
"""
...
from shared.llm import create_client, get_default_model, get_extra_body, with_retry_sync
from CoT.prompt_freeform import COT_FREEFORM_SYSTEM_PROMPT, build_user_message
from CoT.extract_response_freeform import extract_response
...
def solve_task(client: OpenAI, task: Task, test_index: int = 0, provider: str = "openrouter") -> tuple[str, dict]:
    ...
    resp = with_retry_sync(
        lambda: client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": COT_FREEFORM_SYSTEM_PROMPT},
...
def evaluate(
    ...
    log(f"Initializing client (provider={provider}, variant=freeform)...")
    ...
    log(f"Evaluating {len(ids)} tasks from '{split}' split (workers={workers})")
    log(f"{'='*60}")
    ...
    db = EvalClient()
    run_id = db.record_baseline_run(
        "CoT", split, all_results, token_usage,
        extra={"variant": "freeform", "provider": provider},
    )
    db.close()
    if run_id is not None:
        log(f"Recorded CoT (freeform) run {run_id} in DB")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="CoT free-form variant for ARC")
    ...
    parser.add_argument("--provider", type=str, default="openrouter",
                        choices=["deepseek", "openai", "openrouter", "openrouter-friendli",
                                 "openrouter-qwen3", "openrouter-llama-3.3",
                                 "openrouter-llama-4-maverick", "openrouter-kimi-k2"])
